...
// Серверний рендеринг: підтягуємо глобальну історію
export async function getServerSideProps() {
  try {
    // ТУТ БУДЕ ЗАПИТ ДО БАЗИ АБО API ДЛЯ ОТРИМАННЯ 20 ОСТАННІХ VIN
    // Наприклад: const res = await fetch('https://your-domain.com/api/recent');
    // const serverHistory = await res.json();
    
    // Тимчасово залишаю порожній масив, доки ти не скажеш точний метод підключення
    const serverHistory = []; 
    ...
    return { props: { serverHistory: [] } };
  }
}

export default function Home({ serverHistory }) {
  ...
  return (
    <div className="container" dir={t.dir}>
      ...
      <main className="main">
        ...
        {/* Тепер історія тягнеться глобально з сервера */}
