...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // 2. Робимо запит до твого API для отримання реальних кодів
    fetch('/api/vins')
      .then(res => res.json())
      .then(data => {
        // Підтримуємо як масив ['VIN1', 'VIN2'], так і об'єкт { vins: [...] }
        const codes = Array.isArray(data) ? data : (data.vins || []);
        if (codes.length > 0) {
          // Беремо останні 20 штук
  ...
  const handleSearch = (searchVin = vin) => {
    ...
    if (fVin.length >= 8) {
      // Переходимо на сторінку коду (запис нового коду у файл відбудеться вже на бекенді)
  ...
  return (
    <div className="container" dir={t.dir}>
      ...
      <main className="main">
        ...
        {/* Секція "Останні пошуки" з'явиться тільки якщо API повернув хоча б один код */}
        {history.length > 0 && (
