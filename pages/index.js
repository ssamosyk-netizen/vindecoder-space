...
export default function Home() {
  ...
  useEffect(() => {
    ...
    fetch('/api/vins')
      ...
      .then(data => {
        ...
        if (codes.length > 0) {
          // Фільтруємо старе сміття, якщо воно вже потрапило в твій Redis
  ...
  // ЖОРСТКА ВАЛІДАЦІЯ VIN-КОДУ (РЕЖИМ ПАРАНОЇ)
  const isValidVin = (testVin) => {
    if (!testVin || testVin.length !== 17) return false;
    
    // Перевірка 1: Тільки букви/цифри, без I, O, Q
    const vinRegex = /^[A-HJ-NPR-Z0-9]{17}$/i;
    if (!vinRegex.test(testVin)) return false;

    // Перевірка 2: Мінімум 5 УНІКАЛЬНИХ символів (щоб відсіяти 33222222222211111)
    const uniqueChars = new Set(testVin.split(''));
    if (uniqueChars.size < 5) return false;

    // Перевірка 3: Блокуємо "залипання" клавіатури (6 однакових символів підряд)
    // Виняток для нуля "0", бо серійні номери часто бувають типу "000001"
    if (/([1-9A-HJ-NPR-Z])\1{5,}/i.test(testVin)) return false;

    // Перевірка 4: Блокуємо тупий набір "12345678901234567"
  ...
  const handleSearch = (searchVin = vin) => {
    ...
    if (isValidVin(fVin)) {
      ...
      // Записуємо тільки після жорсткої валідації
      fetch('/api/vins', {
        method: 'POST',
